...
ToF adds a number of custom settings to the engine configuration. While some of these might be omitted, we strongly suggest not to do so, as the game might behave unstable.
...
ToF export settings very little particular requirements. In most cases, you can use your own settings file.
...
When preparing a new export, please check, if one of provided files does cover your needs. For example, default *desktop* settings should be fine for any desktop environment, while *android* settings should be a baseline for any platform with fixed resolution and enforced fullscreen.

Provided export settings are often just defaults provided by the Godot Engine. Android export settings have all of the required permissions already configured. Please note, that android export is only provided as a .dist file, and additional release configuration (versioning, keystore) must be filled in.
