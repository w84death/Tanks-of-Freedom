...
func get__s(api, resource, expect_json = true):
	return self.make_request(api, resource, HTTPClient.METHOD_GET, null, expect_json)

func post__s(api, resource, data = "", expect_json = true):
...
func make_request(api, resource, method, data, expect_json = true):
	...
	var err = self.http_client.connect_to_host(api, self.api_port, self.api_use_ssl)
