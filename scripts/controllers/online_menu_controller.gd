...
var multiplayer__s = preload("res://scripts/controllers/multiplayer_menu_controller.gd").new()
var controls
var background
...
func _initialize():
	self.multiplayer__s._init_bag(self.bag)
	self.bind()
...
func show():
	...
	if self.bag.root.settings['online_player_id'] == null:
		...
		self.multiplayer__s.refresh_matches_list()
...
func upload_custom_map(map_name, is_remote = false):
	...
	if self.bag.map_list.maps[map_name]['completed'] or not ProjectSettings.get_setting('tof/map_upload_win'):
